...
```{r}
birds_new <- group_summarize(birds)
```
...
```{r}
birds_really_new <- group_summarize(birds, type)

ggplot(birds_really_new, aes(egg_mass_median, male_mass_median))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()
```
...
```{r}
birds_really_really_new <- filter(birds_really_new, n>=5)
```
...
ggplot(birds_really_new, aes(egg_mass_median, male_mass_median))+
  ...
  scale_x_log10()+
  scale_y_log10()+
  geom_point(color = "red", data = birds_really_really_new)+
  geom_text_repel(data = birds_really_really_new, aes(label = type, y=male_mass_median * 0.9))
```
